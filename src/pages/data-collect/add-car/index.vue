  ...
  <view class="add-car-main-wrapper">
    <u-form :model="form" ref="addCarForm" label-width="auto" :error-type="['toast']">
      <u-card title="基本信息" class="custom-card required" :show-foot="false">
        <view slot="body">
          <u-form-item label="车辆名称" prop="nickname">
            <u-input v-model="form.nickname" input-align="right" placeholder="请输入车辆名称" />
          </u-form-item>
        </view>
      </u-card>
      ...
      <u-card title="车辆信息" class="custom-card required" :show-foot="false">
        <view class="" slot="body">
          ...
          <u-icon
            v-if="form.brand._id && form.series._id && form.car._id"
            class="switch-car"
            name="switch"
            custom-prefix="custom-icon"
            size="30"
            @click="handleGoSelectBrand"
          />
          <view
            v-if="!form.brand._id || !form.series._id || !form.car._id"
            class="add-car-wrapper"
            @click="handleGoSelectBrand"
          >
          ...
          <view v-else class="car-info-wrapper">
            <view class="car-info-title">
              {{ form.series.series_name + ' ' + form.car.car_year + '款 ' + form.car.car_name }}
            </view>
            <view class="car-info-detail">
              <view class="car-info-items">
                ...
                <view class="car-info-item">
                  {{
                    '车重：' +
                    form.car._cp.curb_weight.value +
                    'KG ' +
                    '油箱：' +
                    form.car._cp.oil_tank_volume.value +
                    'L'
                  }}
                </view>
                <view class="car-info-item">
      ...
      <u-card title="其他（可选）" class="custom-card" :show-foot="false">
        <view class="" slot="body">
          ...
          <u-form-item label="提车日期">
            ...
            <u-input
              v-model="form.purchase_date.result"
              input-align="right"
              placeholder="请输入提车日期"
              :clearable="false"
              :disabled="true"
          ...
          <u-form-item label="提车地点">
            <u-input
              v-model="form.purchase_loc.name"
              input-align="right"
              placeholder="请输入提车地点"
              :clearable="false"
              :disabled="true"
          ...
          <u-form-item label="提车价格">
            <u-input
              v-model="form.purchase_price.result"
              input-align="right"
              placeholder="请输入提车价格"
              :clearable="false"
              :disabled="true"
    ...
    <u-modal
      v-model="isSelectPriceShow"
      :show-title="false"
      :show-cancel-button="true"
      @confirm="handlePriceModalOk"
      @cancel="isSelectPriceShow = false"
    >
